"""Canonical text normalization and legal signal extraction for LegalIR."""
...
def clean_legal_text(text: str) -> str:
    """Normalize whitespace and standard punctuation in legal text using NFC."""
...
def prettify_doc_title(name: str) -> str:
    """Turn slug-style document filenames into clean Vietnamese titles."""
    if not name or name == "None" or name == "nan":
        return ""
    name = str(name).strip()
    name = re.sub(r'-\d{4,8}$', '', name)
    m = re.search(
        r'(Nghi-dinh|Thong-tu|Quyet-dinh|Luat|Bo-luat)-(\d+)-(\d+)-([A-ZĐa-z]+-[A-ZĐa-z]+|[A-ZĐa-z]+)',
        name,
        re.I,
    )
    if m:
...
def normalize_question(text: str) -> str:
    """Clean question text and strip excess punctuation for retrieval matching."""
...
def extract_legal_signals(text: str) -> dict[str, list[str]]:
    """Extract legal document numbers, article numbers, clauses, and years."""
...
def tokenize_vietnamese(text: str) -> str:
    """Tokenize Vietnamese text with PyVi if available, otherwise normalize whitespace."""
    cleaned = clean_legal_text(text)
