...
def build_legalir_notebook() -> dict:
    ...
    cell_0 = {
        ...
        "source": [
            ...
            "- **Dataset Source**: `phucdangg/legalir-task1-clean-data` (8,532 documents, 7,000 train queries, 999 public test queries).\n",
            "\n",
    ...
    cell_1 = {
        ...
        "source": [
            ...
            "RUN_MODE = os.environ.get(\"LEGALIR_RUN_MODE\", \"gpu_smoke\")  # \"gpu_smoke\" (default) or \"full\"\n",
    ...
    # Cell 3: Canonical Dataset & Working Directory Discovery & Strict Identity Check
    cell_3 = {
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            ...
            "\n",
            "# Strict v2 Dataset Identity Verification (P1-4)\n",
            ...
            "\n",
            "df_docs = pd.read_parquet(DATA_DIR / \"documents.parquet\") if (DATA_DIR / \"documents.parquet\").exists() else pd.DataFrame()\n",
            "df_chunks = pd.read_parquet(DATA_DIR / \"chunks.parquet\") if (DATA_DIR / \"chunks.parquet\").exists() else pd.DataFrame()\n",
            "df_queries = pd.read_parquet(DATA_DIR / \"queries_train.parquet\") if (DATA_DIR / \"queries_train.parquet\").exists() else pd.DataFrame()\n",
            "df_qrels = pd.read_parquet(DATA_DIR / \"qrels_train.parquet\") if (DATA_DIR / \"qrels_train.parquet\").exists() else pd.DataFrame()\n",
            "public_data = json.loads(PUBLIC_TEST_FILE.read_text(encoding=\"utf-8\")) if (PUBLIC_TEST_FILE and PUBLIC_TEST_FILE.exists()) else {}\n",
            "\n",
            "n_micro = int((df_chunks[\"granularity\"] == \"micro\").sum()) if \"granularity\" in df_chunks.columns else 0\n",
            "n_macro = int((df_chunks[\"granularity\"] == \"macro\").sum()) if \"granularity\" in df_chunks.columns else 0\n",
            "\n",
            ...
            "print(f\"[+] Documents Count           : {len(df_docs):,} (expected 8,532)\")\n",
            "print(f\"[+] Total Chunks Count        : {len(df_chunks):,} (expected 1,153,876) [Micro: {n_micro:,}, Macro: {n_macro:,}]\")\n",
            "print(f\"[+] Train Queries Count       : {len(df_queries):,} (expected 7,000)\")\n",
            "print(f\"[+] Qrels Count               : {len(df_qrels):,} (expected 7,637)\")\n",
            "print(f\"[+] Public Queries Count      : {len(public_data):,} (expected 999)\")\n",
            "print(f\"[+] Audit Report Valid        : {audit_data.get('is_valid', False)} | Errors: {audit_data.get('errors', [])}\")\n",
            "\n",
            "if RUN_MODE in (\"gpu_smoke\", \"full\"):\n",
            "    if len(df_docs) != 8532:\n",
            "        raise ValueError(f\"Dataset identity mismatch: documents count is {len(df_docs)}, expected 8,532\")\n",
            "    if len(df_chunks) != 1153876:\n",
            "        raise ValueError(f\"Dataset identity mismatch: chunks count is {len(df_chunks)}, expected 1,153,876\")\n",
            "    if len(df_queries) != 7000:\n",
            "        raise ValueError(f\"Dataset identity mismatch: train queries count is {len(df_queries)}, expected 7,000\")\n",
            "    if len(df_qrels) != 7637:\n",
            "        raise ValueError(f\"Dataset identity mismatch: qrels count is {len(df_qrels)}, expected 7,637\")\n",
            "    if len(public_data) != 999:\n",
            "        raise ValueError(f\"Dataset identity mismatch: public queries count is {len(public_data)}, expected 999\")\n",
...
def generate_and_save_notebooks(repo_root: Path | None = None) -> tuple[Path, Path]:
    ...
    # Format JSON with exact deterministic indent
    ...
    print(f"[+] Generated thin 5-cell notebook at {root_nb} ({len(root_bytes):,} bytes)")
    print(f"[+] Synced byte-identical copy at {kernel_nb} ({len(kernel_bytes):,} bytes)")

    return root_nb, kernel_nb


if __name__ == "__main__":
    generate_and_save_notebooks()
